...
pub fn setup_tray(app: &App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let open_file =
        MenuItemBuilder::with_id("tray-open-file", "Open Torrent File...").build(handle)?;
    ...
    let check_updates =
        MenuItemBuilder::with_id("tray-check-updates", "Check for Updates...").build(handle)?;
    let sep2 = PredefinedMenuItem::separator(handle)?;
    let task_list = MenuItemBuilder::with_id("tray-task-list", "Task List").build(handle)?;
    let preferences =
        MenuItemBuilder::with_id("tray-preferences", "Preferences...").build(handle)?;
