  ...
  methods: {
    async processTorrents(fileList = []) {
      ...
      try {
        ...
        this.files = this.previewDisabled ? [] : listTorrentFiles(resolved.files || [])
    ...
    buildPreviewNotice(payload: ResolvedTorrentPayload) {
      ...
      if (reason === 'count' && Number(payload.fileCount) > 0) {
        return this.$t('task.torrent-preview-disabled-count', { count: payload.fileCount })
      }
    ...
    isPreviewFolderLoaded(path = '') {
      if (!path) {
        return Object.prototype.hasOwnProperty.call(this.previewPageStateByFolder, '')
      }
      return Object.prototype.hasOwnProperty.call(this.previewPageStateByFolder, path)
    },
    ...
    normalizePreviewFileIndex(value: any) {
    ...
    normalizePreviewRange(range: PreviewIndexRange): PreviewIndexRange | null {
      ...
      return {
        ...
        end: totalCount > 0 ? Math.min(normalizedEnd, totalCount) : normalizedEnd,
      }
    },
    ...
    subtractPreviewRanges(
      ...
      sourceRanges.forEach((range) => {
        ...
        while (j < subtractRanges.length && subtractRanges[j].end < currentStart) {
          j += 1
        }

        let k = j
        while (k < subtractRanges.length && subtractRanges[k].start <= currentEnd) {
          const cut = subtractRanges[k]
          if (cut.start > currentStart) {
            result.push({ start: currentStart, end: Math.min(currentEnd, cut.start - 1) })
          }
    ...
    countPreviewExcludedInRanges(ranges: PreviewIndexRange[] = []) {
      ...
      ranges.forEach((range) => {
        ...
        while (
          ...
          const overlapStart = Math.max(range.start, this.previewExcludedRanges[cursor].start)
          const overlapEnd = Math.min(range.end, this.previewExcludedRanges[cursor].end)
    ...
    previewFolderSelectionState(row: PreviewTreeRow): boolean | 'indeterminate' {
      if (!row || row.type !== 'folder') {
        return false
      }
    ...
    togglePreviewFolderSelection(row: PreviewTreeRow, checked: boolean | 'indeterminate') {
      if (!row || row.type !== 'folder') {
        return
      }
      const ranges = this.getPreviewRowSelectableRanges(row)
    ...
    buildSelectFileRangesFromExcluded() {
      ...
      const excluded = this.normalizePreviewExcludedRanges(this.previewExcludedRanges)
      ...
      if (start <= totalCount) {
        selected.push(start === totalCount ? `${start}` : `${start}-${totalCount}`)
      }
    ...
    emitPreviewSelectionChange() {
      ...
      this.$emit('change', this.currentTorrentPath, this.buildSelectFileRangesFromExcluded())
    },
    togglePreviewFileSelection(row: PreviewTreeRow, checked: boolean | 'indeterminate') {
      if (!row || row.type !== 'file') {
        return
      }
      const index = this.normalizePreviewFileIndex(row.index)
    ...
    mergePreviewItems(existing: ResolvedTorrentItem[] = [], incoming: ResolvedTorrentItem[] = []) {
    ...
    async loadPreviewFolderPage(parentPath = '', append = false) {
      ...
      this.previewLoadingFolders = [...this.previewLoadingFolders, normalizedParentPath]
      const parseVersion = this.parseVersion
      try {
        const resolved = await this.resolveTorrentByPath(this.currentTorrentPath, this.name, {
          forcePreview: true,
        ...
        if (!normalizedParentPath) {
          ...
          const existingItems = this.previewChildrenByFolder[normalizedParentPath] || []
          this.previewChildrenByFolder = {
            ...this.previewChildrenByFolder,
        ...
        this.previewTotalCount = Number(resolved.fileCount || this.previewTotalCount)
      } catch (err: any) {
        logger.warn('[Motrix] load folder preview failed:', err?.message || err)
        this.$msg.error(this.$t('task.new-task-torrent-required'))
      } finally {
    ...
    async handlePreviewAnyway() {
      if (!this.previewDisabled || this.previewLoading || !this.currentTorrentPath) {
        return
      }
      ...
      try {
        ...
        this.resetPreviewState()
        this.previewRootItems = this.previewDisabled ? [] : resolved.items || []
        this.previewTotalCount = Number(resolved.fileCount || 0)
        if (!this.previewDisabled) {
        ...
        logger.warn('[Motrix] force preview torrent failed:', err?.message || err)
        this.$msg.error(this.$t('task.new-task-torrent-required'))
      } finally {
        this.previewLoading = false
      }
    ...
    async triggerFileInput() {
      ...
      try {
        ...
        logger.warn('[Motrix] pick torrent path failed:', err?.message || err)
      }
    },
    onDrop(event) {
      if (this.isBusy) {
      ...
      const path = `${(file as any).path || ''}`.trim()
      if (!path) {
        this.$msg.error(this.$t('task.new-task-torrent-required'))
        return
      }
