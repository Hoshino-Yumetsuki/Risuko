...
export default {
  ...
  computed: {
    ...
    height() {
      ...
      const contentHeight = rowCount > 0 ? atomHG * (rowCount - 1) + atomHeight : 0
    ...
    cellStatuses() {
      ...
      if (Array.isArray(cellPercents) && cellPercents.length >= len) {
        return cellPercents.slice(0, len).map((percent) => percentToStatus(percent))
      }
      ...
      for (let bitIndex = 0; bitIndex < rawBitLen; bitIndex++) {
        ...
        const bucket = Math.min(Math.floor((bitIndex * len) / rawBitLen), len - 1)
        sums[bucket] += bit
        counts[bucket] += 1
      }
  ...
  methods: {
    buildAtom(index) {
      const { xOffset, yOffset, atomWG, atomHG, gridColumnCount, cellStatuses } = this
