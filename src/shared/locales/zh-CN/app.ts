export default {
	"task-list": "任务列表",
	"add-task": "新建任务",
	"primary-navigation": "主导航",
	"nav-tasks": "任务",
	"nav-rss": "RSS",
	"nav-health": "健康",
	"nav-settings": "设置",
	"nav-share": "分享",
	"nav-stats": "统计",
	"nav-account": "账户",
	"android-back-again-to-quit": "再次返回以退出 Risuko",
	"android-storage-access-required":
		"请授予所有文件访问权限，然后重新选择下载文件夹。",
	about: "关于 Risuko",
	preferences: "偏好设置...",
	"check-for-updates": "检查更新...",
	"check-updates-now": "立即检查",
	"checking-for-updates": "正在检查更新...",
	"check-for-updates-title": "检查更新",
	"update-available-message": "发现新版本，是否现在更新？",
	"update-not-available-message": "已是最新版",
	"update-downloaded-message": "更新下载完成，应用程序将退出并开始更新...",
	"update-error-message": "检查更新失败",
	"engine-damaged-message": "引擎损坏，请重新安装 : (",
	"engine-missing-message": "引擎缺失，请重新安装 : (",
	"system-error-title": "系统错误",
	"system-error-message": "应用启动失败: {{message}}",
	hide: "隐藏 Risuko",
	"hide-others": "隐藏其他",
	unhide: "显示全部",
	show: "显示 Risuko",
	"quick-panel": "快捷面板",
	"flyout-active-count": "{{count}} 个进行中",
	"flyout-empty": "暂无任务",
	"flyout-no-match": "没有匹配的任务",
	"total-transfer-speed": "总传输速度",
	quit: "退出 Risuko",
	"quit-confirm": "确定要退出 Risuko 吗？",
	"quit-confirm-active": "还有 {{count}} 个下载任务正在进行中，确定要退出吗？",
	"under-development-message": "该功能开发中...",
	yes: "是",
	no: "否",
	save: "保存",
	reset: "放弃",
	cancel: "取 消",
	submit: "提 交",
	browse: "浏览",
	gt1d: "超过一天",
	hour: "时",
	minute: "分",
	second: "秒",
	"legal-title": "欢迎使用 Risuko",
	"legal-body": "开始使用前，请阅读我们的服务条款和隐私政策。",
	"legal-terms": "服务条款",
	"legal-privacy": "隐私政策",
	"legal-agree": "同意并继续",
	"legal-agree-note": "继续即表示您同意上述条款。",
	"legal-save-failed": "无法保存您的选择，请重试。",
	"open-url": "打开 URL",
	"open-url-failed": "无法打开 URL",
	"increase-value": "增加数值",
	"decrease-value": "减少数值",
	"previous-month": "上个月",
	"next-month": "下个月",
	"update-checking": "正在检查更新...",
	"update-unavailable": "暂无可用更新",
	"update-available": "有可用更新",
	"update-download": "下载更新",
	"update-cancelled": "更新已取消",
	"update-install": "安装更新",
	"update-relaunch": "重启以完成更新",
	"update-progress": "更新进度: {{progress}}%",
	"update-error": "更新错误: {{error}}",
	"update-unsigned": "此版本无法使用更新",
};
